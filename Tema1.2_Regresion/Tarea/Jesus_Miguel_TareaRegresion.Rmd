...
A continuación se muestra lo que devuelve esta función al aplicarse a nuestros datos. Obtendremos primeramente una especie de tabla en el que se muestran los mejores modelos (`nbest = 1`) según el número de variables inmiscuidas (de 1 hasta `nvmax`), junto con las variables que este emplea.
...
numeric_columns <- colnames(cars)[sapply(cars, is.numeric)]
...
ajuste.todo <- regsubsets(formula, data = cars, nvmax = nvmax, nbest = 1)
...
```{r}
# Obtener el modelo con la mejor métrica BIC
mejor_modelo_bic <- resumen$which[which.min(resumen$bic), ]

# Obtener nombres de las variables incluidas en el mejor modelo
variables_incluidas <- names(which(mejor_modelo_bic))
variables_incluidas <- variables_incluidas[variables_incluidas != "(Intercept)"]

# Crear la fórmula con términos cuadráticos y variables seleccionadas
formula <- reformulate(variables_incluidas, response = "Price")
...
```{r seleccion_modelo_regsubsets}
mejor_modelo_bic <- resumen$which[which.min(resumen$bic), ]

variables_incluidas <- names(which(mejor_modelo_bic))

# Arreglos para contener la variable de tipo factor entera y no solo una de sus opciones:
...
#variables_incluidas_arreglado <- variables_incluidas_arreglado[variables_incluidas_arreglado %in% colnames(cars)]

# Modelo de regresión lineal múltiple obtenido:
modelo_final <- lm(Price ~ . , data = cars[, c("Price", variables_incluidas_arreglado)])
summary(modelo_final)
```
...
El método seleccionado es el perteneciente a `r metodo_menor_valor`, donde se obtuvo un `AIC` de `r valor_minimo_AIC` y utilizando como variables predictoras `r colnames(get(paste("traza_", metodo_menor_valor, sep = ""))$model)`.
...
Se debe ahora repetir lo hecho en el apartado anterior.
...
ajuste_todo <- lm(Price ~ ., data = cars)
traza_both <- step(ajuste_todo, direction = 'both', trace = 0)
```
